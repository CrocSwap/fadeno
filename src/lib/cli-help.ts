...
const TOP_LEVEL: Record<string, PageSeed> = {
  ...
  'dispatch-open': page('The spawn wrapper for a spawn the host is about to make: open it on the host lane, or hand it to the command lane.', 'fadeno dispatch-open --archetype <name> [--name <n>] [--model <ref>] [--lane auto|host|command] [--shared] [--from <ref>] [--session-id <id>] [--parent <id> | --parent-transcript <path>] [--harness <id>] [--agent-id <id>] (--prompt-file <path> | stdin | --prompt-sealed <reason> | --dry-run) [--json]', [
  ...
  'dispatch-stop': page('Record that a host-lane dispatch\'s agent stopped, and what its tree holds.', 'fadeno dispatch-stop [<name|id>] [--agent-id <id>] [--transcript <path>] [--message-file <path> | stdin] [--agent-cwd <dir>] [--json]', [
...
const PAGE_OPTIONS: Record<string, readonly string[]> = {
  ...
  'dispatch-stop': withGlobals('--agent-id', '--transcript', '--message-file', '--agent-cwd', '--json'),
