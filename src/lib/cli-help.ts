...
const TOP_LEVEL: Record<string, PageSeed> = {
  ...
  models: MODELS_PAGE,
  model: aliasPage(MODELS_PAGE, ['fadeno model [<name>]', 'fadeno model --driver <alias>', 'fadeno model add <alias> <provider/id>'], '`fadeno model` is an alias for `fadeno models`.'),
  ...
  dispatches: page('Inspect or recover command dispatches.', ['fadeno dispatches [--tail <count>] [--json] [--bakeoffs]', 'fadeno dispatches --output <id|last|tag:<tag>> [--wait <seconds>]', 'fadeno dispatches --cancel <id|tag:<tag>> | --merge <id|tag:<tag>>'], ['`--output` writes the saved snapshot bytes verbatim to stdout; use `--wait` only for a completion row.']),
  'shadow-apply': page('Apply a selected shadow arm diff.', 'fadeno shadow-apply <pair-id|dispatch-id> [--arm challenger|primary] [--check]'),
  bakeoff: page('Measure or adjudicate a shadow pair.', ['fadeno bakeoff <pair-id|dispatch-id> [--measure-only] [--judge <ref>] [--via <driver>] [--evidence inlined|explored]', 'fadeno bakeoff <pair-id|dispatch-id> --prepare [--evidence inlined|explored]', 'fadeno bakeoff <pair-id|dispatch-id> --record --comparison <file> --adversarial <file> [--evidence inlined|explored]'], ['`--measure-only` records no adjudication; `--prepare` writes blinded prompts; `--record` requires both host-delivered judgment files.']),
...
const OPTION_HINTS: Record<string, string> = {
  ...
  '--dispatch-id': 'Immutable host dispatch id', '--driver': 'Live backend driver', '--events': 'Include raw event timeline',
  ...
  '--verbose': 'Include diagnostic detail', '--version': 'Show Fadeno version', '--via': 'Delivery driver',
...
const OPTION_FORMS: Record<string, string> = {
  ...
  '--archetype': '--archetype <name>', '--model': '--model <ref>', '--via': '--via <driver>',
  '--prompt-file': '--prompt-file <path>', '--output': '--output <path>', '--bind': '--bind <role=executor>',
  '--tool': '--tool <name>', '--driver': '--driver <alias>', '--input': '--input <name=path>',
...
const PAGE_OPTIONS: Record<string, readonly string[]> = {
  ...
  models: withGlobals('--driver', '--json'),
  model: withGlobals('--driver', '--json'),
  dial: withGlobals('--via', '--session', '--user', '--repo', '--json'),
  shadow: withGlobals('--via', '--rate', '--n', '--json'),
  steering: withGlobals(),
  dispatch: withGlobals('--archetype', '--model', '--role', '--via', '--prompt-file', '--tag', '--shadow', '--timeout', '--isolate', '--shared', '--ignored-output', '--diagnostics', '--no-brief'),
  ...
  dispatches: withGlobals('--tail', '--json', '--bakeoffs', '--output', '--wait', '--tag', '--cancel', '--merge'),
  'shadow-apply': withGlobals('--arm', '--check'),
  bakeoff: withGlobals('--measure-only', '--evidence', '--prepare', '--record', '--comparison', '--adversarial', '--json', '--judge', '--via'),
  ...
  'dial shadow': withGlobals('--via', '--rate', '--n', '--json'),
