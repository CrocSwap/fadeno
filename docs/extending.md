...
- **`ignored_output_discarded` receipts, and where they are read.** An
  isolated delivery merges back through `git add -A`, which respects
  `.gitignore`, so anything the executor built at an ignored path was staged
  by nothing and died with the worktree. The scan lands on the receipt as
  ...
  because a report describing files at those paths is describing files that no
  longer exist.
