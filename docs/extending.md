...
- **`concurrent_write` receipts.** At the terminal receipt a delivery
  ...
  which is worse.
- **Where the stamp is READ.** Detection that nobody projects is the same as no
  detection. `fadeno verify` reports a `concurrent-writes` finding — `warn`,
  never `fail`: an overlap is not proof of damage, and gating on one would
  block `fadeno evidence` on ordinary runs and teach readers to reach for
  `--allow-failed`. `fadeno show` renders a `concurrent writes` section under
  the workflow. `fadeno dispatches` renders it inline (the only home of an
  ad-hoc dispatch's stamp) and `--output` states it beside the bytes rather
  than prefixing it onto them — the in-band channel is reserved for findings
  that make the report itself false.
- **`ignored_output_discarded` receipts, where they are read, and what the
  kernel does about them.** An isolated delivery merges back through
  `git add -A`, which respects `.gitignore`, so anything the executor built at
  an ignored path is staged by nothing and no diff carries it out. The scan
  lands on the receipt as
  ...
  `workspace` / `workspace_retained` say the directory survived. `fadeno clean
  --force` is what reclaims it. One predicate decides both the field and the
  teardown, and a truncated scan counts as "not clean": deleting a directory
  on a listing that just said it could not enumerate what was in it is the
...
**Isolated host workspaces.** `fadeno dispatch-prepare <run> <dispatch-id> --isolate` is the opt-in isolated-host primitive: it validates `run` and `dispatch-id` against `HOST_WORKSPACE_SEGMENT_RE`, guards against traversal/symlink escape, serializes with `.fadeno/local/.host-workspace.lock`, creates an idempotent detached worktree at `.fadeno/local/host-worktrees/<run>/<dispatch-id>` from `HEAD`, replays the caller's tracked and untracked/unignored changes as a synthetic baseline commit under the short read-window lease, and atomically records `workspace_mode: isolated` state at `.fadeno/local/host-workspaces/<run>/<dispatch-id>.json` (`workspace`, `base_commit`, `prepared_at`, plus `diff_snapshot`/`diff_bytes`/`finalized_at` after collection). After preparation, `dispatch-prompt` includes `workspace_mode: isolated` and the absolute workspace path (prompt bytes and digest unchanged); `dispatch-start` stamps `workspace_mode: isolated`/`workspace`/`base_commit` on `actor_dispatched` and records an overlap window (there is no shared writer lease to bypass any more — see `concurrent_write` below); `dispatch-complete`/`dispatch-fail` collect a binary staged diff of only post-baseline host changes to `.fadeno/local/outputs/host-isolated-<run>-<dispatch-id>.diff` before the terminal receipt (proving the worktree is the registered linked worktree before any `git add`/`diff` or removal), stamping `workspace_mode: isolated` plus `workspace`/`base_commit` and `diff_snapshot`/`diff_bytes` only when a diff was actually collected from the proven worktree. `dispatch-fail` degrades to a terminal receipt without diff keys whenever evidence is absent, unverifiable, or unrecoverable — including a missing or malformed machine-local state file — while a collection failure with the state present still refuses, preserving the worktree for retry; `dispatch-complete` may recover and collect from a verified ledger-named worktree when the state vanished but still refuses success when evidence cannot be collected. The worktree is removed only after the receipt is durable, only when proven registered, and only when a pre-teardown `scanIgnoredOutput` positively says it holds no gitignored content — a host receipt carries the same `ignored_output_discarded` field the command lane does, with `retained_at`, and the refusal to delete is enforced inside `removeHostWorkspaceByPath` so a completion, a failure, a cancel and a recovery cannot each forget it (failures and degraded paths preserve the worktree for manual recovery; idempotent terminals reuse the receipt and retry cleanup only when verified; nothing auto-merges). `fadeno show` projects `workspaceMode`/`workspace`/`baseCommit`/`diffSnapshot`/`diffBytes` on `HostRequestView` (ledger-first, prepared-but-not-started degrades to isolated via machine-local state, missing state → shared/null, never throws) and prints isolated facts as non-gating observability; `verify` checks only ledger consistency and never requires machine-local state. See `src/lib/host-workspace.ts`, `src/commands/dispatch-prepare.ts`, `src/lib/host-dispatch.ts`.

**Runless host dispatch.** Everything above is run-scoped: `dispatch-prepare` requires a `<run>` and every receipt writer reads a `.fadeno/runs/<run>/` ledger, so before `fadeno dispatch-open` the only way to get an isolated worktree plus a terminal receipt for ad-hoc work was the COMMAND lane — the asymmetry that pushed whole parallel campaigns onto the lossy lane while Fadeno's stated preference was the host lane. `fadeno dispatch-open [--archetype <name>] [--tag <handle>] [--note <text>]` calls the SAME `prepareHostWorkspace` primitive with the literal path segment `adhoc` (`ADHOC_HOST_SCOPE`) filling the run slot — that argument is validated as a `HOST_WORKSPACE_SEGMENT_RE` segment, never resolved as a run, and no `YYYY-MM-DD-HHMM-slug` run id can collide with it — so the worktree, machine-local state and diff live at `.fadeno/local/host-worktrees/adhoc/<dispatch-id>`, `.fadeno/local/host-workspaces/adhoc/<dispatch-id>.json` and `.fadeno/local/outputs/host-isolated-adhoc-<dispatch-id>.diff` with the same traversal/symlink guards, the same `.host-workspace.lock`, and the same dirty-workspace baseline replay. It mints a uuid dispatch id, opens a `host-dispatch` overlap window, and appends `adhoc_host_dispatch_requested` to `.fadeno/dispatches.jsonl`; the host then spawns its in-session agent against the printed absolute workspace path. `fadeno dispatch-close <id|tag:<handle>|last>` collects the diff, merges it back with `settleIsolatedWork` (withheld by `--no-merge`, and by a `--reason <text>` failure close), stamps `concurrent_write` from the window log, closes the window, and appends one `adhoc_host_dispatch_closed` receipt carrying `outcome` (`ok`/`failed`, STATED by the host and never derived — there is no exit code, because Fadeno never held the process), `diff_snapshot`/`diff_bytes`, `primary_merge`, and the retained `workspace`. There is no `dispatch-start` on this lane and therefore one terminal receipt rather than a complete/fail pair. The worktree is torn down on exactly one condition — the work landed in the caller's tree — and every other ending retains it and names it. The evidence lives in `.fadeno/dispatches.jsonl` rather than a synthesized run ledger because that log has one entry reader (`foldEvidenceRow`) where `.fadeno/runs/` has many consumers and a `run.schema.json` that requires a `playbook`; both new event names ship with their `foldEvidenceRow` cases, since a row kind that reader does not handle counts as unreadable damage. `fadeno verify` audits run ledgers only, so it has nothing to say about an ad-hoc host dispatch — handed one's id it names what the id is, whether it is open or closed, and points at `fadeno dispatches`, rather than answering "No run matching". See `src/commands/dispatch-adhoc.ts`.
