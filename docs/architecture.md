...
- **`dispatch-adhoc.ts`** — the RUNLESS half of the host dispatch protocol: `fadeno dispatch-open` / `fadeno dispatch-close`. Everything above is run-scoped — `dispatch-prepare` requires a `<run>` and reads a `.fadeno/runs/<run>/` ledger — so before this, a director doing ad-hoc parallel work could only get isolation plus receipts from the COMMAND lane, and that asymmetry is what pushed whole campaigns onto the lossy lane. `dispatch-open` calls the same `prepareHostWorkspace` primitive with `ADHOC_HOST_SCOPE` (`adhoc`) filling the run path segment — a namespace no `YYYY-MM-DD-HHMM-slug` run id can reach — so the worktree, machine-local state and diff land at `.fadeno/local/host-worktrees/adhoc/<dispatch-id>`, `.fadeno/local/host-workspaces/adhoc/<dispatch-id>.json` and `.fadeno/local/outputs/host-isolated-adhoc-<dispatch-id>.diff`. It mints a uuid dispatch id, opens a `host-dispatch` overlap window, and appends one `adhoc_host_dispatch_requested` row to `.fadeno/dispatches.jsonl` — the command lane's own evidence log, chosen over a synthesized run ledger because that file has one entry reader (`foldEvidenceRow`) where `.fadeno/runs/` has many consumers and a `run.schema.json` that requires a `playbook`. `dispatch-close` collects the diff, merges it back with `settleIsolatedWork` (withheld by `--no-merge` and by a `--reason` failure close), stamps `concurrent_write` from the window log, closes the window, and appends one `adhoc_host_dispatch_closed` receipt carrying `outcome`, `diff_snapshot`/`diff_bytes`, `primary_merge` and the retained `workspace`. The worktree is torn down on exactly one condition — the work landed in the caller's tree — and every other ending retains it and names it. There is no `dispatch-start` on this lane (the host spawns out of band) and therefore ONE terminal receipt rather than a complete/fail pair. No run ledger means `fadeno verify` has nothing to audit; handed an ad-hoc dispatch id it says so and points at `fadeno dispatches` instead of answering "No run matching".
...
- **`receipt-attestations.ts`** — the one parser for the two receipt fields
  ...
  that `git add -A` never staged and no diff carried out of a worktree). Both
  were written by the kernel and read by nobody — the first echoed onto a
  stdout the recover-by-tag path discards, the second reaching only the
  `dispatches` listing, which is not where a `data/research/` deliverable was
  noticed missing. Surfaces render differently and parse identically: what
  costs work is not two readers wording a finding differently, it is two
  readers disagreeing about whether there is one. "I could not tell" — a
  truncated listing, an unsettled intersection, a legacy row with no
  completeness flag — never comes back spelled the way "there was nothing" is.
