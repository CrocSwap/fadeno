...
omp follows the OpenCode promotion path, on direct dogfood evidence: before any
adapter existed, an omp session already discovered this repo's `.agents/skills/`
tree and the generated Claude plugin's skills and agents. `fadeno init --omp`
makes that support explicit while keeping the shared playbooks, schemas,
vocabulary, and sigil-free skill bodies unchanged:

| Concern | omp (`--omp`) |
|---|---|
| Skill dir | `.agents/skills/<name>/` — the cross-harness standard dir, shared with Codex and OpenCode |
| Bootstrap file | `AGENTS.md` (read natively by omp) |
| Invocation | description-invoked skills; omp also registers a native `/skill:<name>` command per skill |
| Subagent defs | `.omp/agents/*.md` — role agents plus bash-only `dispatch-*` proxies (`name` + `description` frontmatter required) |
| Enforcement | common CI/pre-commit scaffold; no omp-specific hook scaffold |
| Route lane | no `routes.omp:` section ships; `current-host` is the base lane, and dialing a model whose driver has no declared `routes.omp` entry fails loudly at compile time |

Steering is intentionally unsupported at this tier: omp's extension-module
surface (a `tool_call` handler may rewrite spawn arguments) is the eventual seam
for Claude-style dispatch steering, deferred pending dogfood evidence. The
plugin surface (`plugin-omp/`, generated by `fadeno plugin --omp`) carries
full-named skills — omp registers `/skill:<name>` itself, so no commands
component ships — the role agents and dispatch proxies, and the bundled CLI;
its marketplace catalog lives at `.omp-plugin/marketplace.json`.
