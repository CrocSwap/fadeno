...
```bash
FADENO_HARNESS=omp "${FADENO_CLI:-fadeno}" dispatch --archetype judge --tag judge-<slug> <<'FADENO_PROMPT'
...
(`judge-parse-retry-header`). The tag is your handle on this dispatch, and the
...
and the tag (`FADENO_HARNESS=omp "${FADENO_CLI:-fadeno}" dispatch --archetype judge --shared --tag judge-<slug> <<'FADENO_PROMPT'`)
...
3. If the command exits non-zero, report its output verbatim and state
   plainly that the dispatch failed. Do NOT perform the evaluation yourself
...
4. If the dispatch call is killed or times out, the result is UNKNOWN — not
   ...
   ```bash
   FADENO_HARNESS=omp "${FADENO_CLI:-fadeno}" dispatches --output tag:judge-<slug> --wait 120
   ```
