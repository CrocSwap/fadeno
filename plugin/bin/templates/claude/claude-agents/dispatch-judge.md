...
1. Write the ENTIRE task prompt you received — verbatim, every line, no
   ...
   f=$(mktemp .fadeno/local/prompts/judge-XXXXXXXX)
   cat > "$f" <<'FADENO_PROMPT'
...
2. Run the dispatch:
   ...
   ```bash
   "${CLAUDE_PLUGIN_ROOT:+$CLAUDE_PLUGIN_ROOT/bin/}fadeno" dispatch --archetype judge --prompt-file "$f"
   ```
...
4. If the command exits non-zero, report its error output verbatim and state
   plainly that the dispatch failed. Do NOT perform the evaluation yourself as
   a fallback — silently substituting which provider does the work is an
