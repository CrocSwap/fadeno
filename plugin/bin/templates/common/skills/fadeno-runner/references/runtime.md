...
tree and run one at a time. Dispatch/receipt rows keep
...
A materialized role agent supplies its own baked `--host-executor`
automatically. The top-level coordinator has none to supply, so its own
resolve call can only ever land on `mode=command` or `mode=restart_required`
for a locked request — never `mode=host`. That is a statement about the
...
- **tool_call** — Invoke the named capability (e.g. `test_runner`, `diff_loader`).
  If the step's output schema is `test-result` and the tool is registered in the layered `tools:` registry (`executors.yaml` as static argv, plus an optional `timeout`/`timeout_ms` that is parsed and never armed — Fadeno runs no executor under a deadline), use `fadeno tool-run <run> [--tool <name>]` — it executes without a shell under the supervisor, strips harness identity, records an overlap window (there is no writer lease any more), synthesizes a `TestResult` (`passed`/`failed`/`error`), validates, and attributes atomically via the shared execution core (same code as `fadeno drive`). Otherwise (Diff/PostResult, or unregistered), write the result manually and attribute with `fadeno tool-complete <run> --output <artifact-path>` — it records the manifest and then a `tool_recorded` receipt (`recorded_by: host`), so the artifact is anchored in the ledger without claiming the kernel measured anything. The two paths are mutually exclusive per generation (same `tool_call_id` namespace, live-claim scan, and ledger-serialized re-check). A typed result that fails its schema is rejected before `step_started` or `artifact_created` is recorded, leaving the tool step retryable.
