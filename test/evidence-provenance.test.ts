...
function seedCatalog(t: import('node:test').TestContext, extra: Record<string, unknown> = {}): string {
  ...
  writeFileSync(join(root, '.fadeno', 'executors.yaml'), stringifyYaml({
    ...
    harnesses: { codex: { provider: 'openai', command: ['node', '-e', 'process.stdout.write(\'report\')'] } },
    archetypes: { worker: {} },
    dials: { worker: 'probe' },
    ...extra,
  } as any));
...
test('refused row stamped with build version', (t) => {
  const root = seedCatalog(t, { dials: { worker: 'gated' } } as any);
  ...
  assert.equal(w[0]!.format, '1.1');
});
test('row shape: format 1.1, executor = refString, model/model_id/harness/reasoning_effort/dial', (t) => {
  const root = seedCatalog(t);
  runDispatch({ archetype: 'worker', prompt: 'x', repoRoot: root, userPathOptions: harnessOpts });
  const w = rows(root);
  for (const row of w) {
